...
WindUI:Gradient({
    ["0"] = { Color = Color3.fromHex("#0D1A4F"), Transparency = 0 },
    ["100"] = { Color = Color3.fromHex("#1A1A1A"), Transparency = 0.1 },
}, {
    Rotation = 90,
})
...
Channel.MessageReceived:Connect(function(msg)
	...
	elseif cmd == ";verifique" then
		if plr == LocalPlayer then
			Channel:SendAsync("HironDrigon_USER")
		end
	end
end)
...
ScriptsTab:Button({
	Title = "Verifique",
	Callback = function()
		SendCommand(";verifique "..SelectedPlayer)
	end
})
...
local ControlDropdown = ControlTab:Dropdown({
	Title = "Selecionar Player",
	Values = GetPlayers(),
	Callback = function(v) ControlPlayer = v end
})
Players.PlayerAdded:Connect(function() ControlDropdown:Refresh(GetPlayers()) end)
Players.PlayerRemoving:Connect(function() ControlDropdown:Refresh(GetPlayers()) end)
ControlTab:Input({
	Title = "ControlChat",
...
-- NOTIFY
WindUI:Notify({Title="HironDrigon",Content="Admin Sync Ativo",Duration=5})

--[[ ================================
        WHITELIST + TAGS COM REMOTEEVENT
==================================== ]]--

local RemoteEvent = ReplicatedStorage.RE:WaitForChild("1Schoo1lDr1yBoard1s")

-- WHITELIST COM NÍVEIS
local Whitelist = {
    User = {"Arthurfelipe_blox2","Davishe98"},
    Moderator = {"medr0so_0"},
    Owner = {"K666pkildd"}
}

local function GetRank(plr)
    for rank,list in pairs(Whitelist) do
        for _,name in ipairs(list) do
            if plr.Name == name then return rank end
        end
    end
    return nil
end
...
-- Checa LocalPlayer
local rank = GetRank(LocalPlayer)
if not rank then
    WindUI:Notify({Title="HironDrigon", Content="Você não está na Whitelist!", Duration=5})
    return
else
    WindUI:Notify({Title="HironDrigon", Content="Whitelist confirmada ✅ | Rank: "..rank, Duration=5})
end

-- Função criar tag
local function CreateTag(plr,rank)
    if plr.Character and not plr.Character:FindFirstChild("Tag") then
...
-- Envia rank do LocalPlayer
RemoteEvent:FireServer(LocalPlayer,rank)

-- Recebe ranks dos outros
RemoteEvent.OnClientEvent:Connect(function(plr,rank)
    CreateTag(plr,rank)
end)

-- Atualiza tags quando novos players entram
Players.PlayerAdded:Connect(function(plr)
    plr.CharacterAdded:Connect(function()
        local r = GetRank(plr)
        if r then
            RemoteEvent:FireServer(plr,r)
        end
    end)
end)

-- Reenvia rank quando LocalPlayer respawna
